...
import cn.jdywl.driver.ui.stage.CarLocationActivity;

/**
 * Created by Administrator on 2016/5/12.
 */
public class DrayageOrderInfoActivity extends BaseActivity {
    public static final String TAG = LogHelper.makeLogTag(DrayageOrderInfoActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_drayage_orderinfo);
        setupToolbar();
        ...
        if (order.getStatus() == OrderStatus.ORDER_TRADING) {
            ...
            tvLocation.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Intent intent = new Intent(DrayageOrderInfoActivity.this, CarLocationActivity.class);
    ...
    private void accept() {
        ...
        if (form == FROM_SMARKET) {
            url = ApiConfig.api_url + ApiConfig.SDRIVERS_ORDER_URL.replace("/", "/" + order.getId() + "/");
        } else if (form == FROM_SPENDING) {
            url = ApiConfig.api_url + ApiConfig.SDRIVERS_CANCEL_URL.replace("/", "/" + order.getId() + "/");
        } else if (form == FROM_SORDER) {
            url = ApiConfig.api_url + ApiConfig.SDRIVERS_FINISH_URL.replace("/", "/" + order.getId() + "/");
        }
        ...
        GsonRequest<StageOrderItem> myReq = new GsonRequest<StageOrderItem>(Request.Method.PUT,
                ...
                new Response.Listener<StageOrderItem>() {
                    ...
                    public void onResponse(StageOrderItem response) {
                        AppConst.dismiss();
                        if (form == FROM_SMARKET) {
                            Toast.makeText(DrayageOrderInfoActivity.this, "接单成功", Toast.LENGTH_SHORT).show();
                        } else if (form == FROM_SPENDING) {
                            Toast.makeText(DrayageOrderInfoActivity.this, "取消成功", Toast.LENGTH_SHORT).show();
                        } else if (form == FROM_SORDER) {
                            Toast.makeText(DrayageOrderInfoActivity.this, "完成订单成功", Toast.LENGTH_SHORT).show();
                        }
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        AppConst.dismiss();
                        Helper.processVolleyErrorMsg(DrayageOrderInfoActivity.this, error);
                    }
                });
        ...
        VolleySingleton.getInstance(DrayageOrderInfoActivity.this).addToRequestQueue(myReq);
    }
