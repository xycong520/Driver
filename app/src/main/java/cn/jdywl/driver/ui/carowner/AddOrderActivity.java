        ...
        getSupportFragmentManager().addOnBackStackChangedListener(
                new FragmentManager.OnBackStackChangedListener() {
                    public void onBackStackChanged() {
                        // Update your UI here.
                        if (creditFragment != null && creditFragment.isVisible()) {
                            updateSubmitBtn("提交");
                        } else {
                            if (orderFragment.rbSrvCredit.isChecked()) {
                                updateSubmitBtn("下一步");
                            } else {
                                updateSubmitBtn("提交");
                            }
    ...
    */
    public void submitOrder(View view) {
        ...
        if (creditFragment == null && orderFragment.rbSrvCredit.isChecked()) {
            ...
            // Add the fragment to the 'fragment_container' FrameLayout
            FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
            transaction.add(R.id.fragment_container, creditFragment);
        ...
        if (creditFragment != null && orderFragment.rgAddtionalSrv.getCheckedRadioButtonId() == R.id.rb_srvCredit) {
            ...
            params.putAll(creditFragment.getParas());
        }
    ...
    public void updatePrice(PriceItem price) {
        if (price != null) {
            tvTotalBill.setText(String.format("总费用：%d元", price.getTotalBill()));
            String carnum = orderFragment.etCarnum.getText().toString();
            if ((!carnum.isEmpty()) && Integer.valueOf(carnum) >= ApiConfig.CAR_SIZE) {
                tvPrice.setText(String.format(getString(R.string.zhengban_price), price.getMarketPrice(), price.getInsurance(), price.getSrvFee() + price.getDeposit()));
            } else {
                tvPrice.setText(String.format(getString(R.string.sanche_price), price.getMarketPrice(), price.getInsurance(), price.getSrvFee() + price.getDeposit()));
            }
        } else {
